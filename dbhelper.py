...
class DBhelper:
    ...
    def view_matches(self, user_id):
        self.mycursor.execute(
            "SELECT * FROM proposals JOIN users ON users.user_id=proposals.juliet_id WHERE juliet_id in (SELECT romeo_id FROM `proposals` WHERE juliet_id={}) AND romeo_id={}".format(
                user_id, user_id))
        data = self.mycursor.fetchall()
        return data
